...
window.iconbitmap("Logo Dice.ico")
...
#save the generated code locally in png format
def save():
    dir = "QR_Codes"
    if not os.path.exists(dir):
        os.makedirs(dir)
...
#save the generated code locally in svg format
def svg():
    dir = "QR_Codes"
    if not os.path.exists(dir):
        os.makedirs(dir)
...
#prevent from saving empty files 
def dummy():
    try:
...
#clear out any entry boxes when the user shifts focus 
...
#repopulate the default text previously inside the entry boxes if nothing is putin
